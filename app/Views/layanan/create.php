...
<div class="container-crud">
  <div class="card-crud card-crud--narrow">
    ...
    <div style="display:flex; justify-content:space-between; align-items:center;">
      <h2 class="card-title">Tambah Cucian</h2>
    ...
    <form method="post" action="/bellaundry/app/Controllers/LayananController.php?aksi=store">

      <div class="<?= !empty($errors['id_pelanggan']) ? 'form-group-error' : '' ?>">
        <label>Cari Pelanggan (ID atau Nama)</label>
        <input type="text" name="id_pelanggan" class="input-field" list="pelanggan-list" placeholder="Ketik ID atau Nama Pelanggan" value="<?= htmlspecialchars($data['id_pelanggan'] ?? '') ?>" required>
      ...
      <div class="<?= !empty($errors['nama_layanan']) ? 'form-group-error' : '' ?>">
        ...
        <select name="nama_layanan" id="nama_layanan" class="input-field" required>
          ...
          <option value="Cuci Setrika Reguler" <?= ($data['nama_layanan'] ?? '') === 'Cuci Setrika Reguler' ? 'selected' : '' ?>>Cuci Setrika Reguler</option>
          <option value="Cuci Setrika Express" <?= ($data['nama_layanan'] ?? '') === 'Cuci Setrika Express' ? 'selected' : '' ?>>Cuci Setrika Express</option>
          <option value="Cuci Lipat Reguler" <?= ($data['nama_layanan'] ?? '') === 'Cuci Lipat Reguler' ? 'selected' : '' ?>>Cuci Lipat Reguler</option>
          <option value="Cuci Lipat Express" <?= ($data['nama_layanan'] ?? '') === 'Cuci Lipat Express' ? 'selected' : '' ?>>Cuci Lipat Express</option>
          <option value="Cuci Saja Reguler" <?= ($data['nama_layanan'] ?? '') === 'Cuci Saja Reguler' ? 'selected' : '' ?>>Cuci Saja Reguler</option>
          <option value="Cuci Saja Express" <?= ($data['nama_layanan'] ?? '') === 'Cuci Saja Express' ? 'selected' : '' ?>>Cuci Saja Express</option>
        </select>
      ...
      <div class="<?= !empty($errors['berat']) ? 'form-group-error' : '' ?>">
        ...
        <input type="number" step="0.1" name="berat" id="berat" class="input-field" placeholder="Masukkan berat cucian" value="<?= htmlspecialchars($data['berat'] ?? '') ?>" required>
      ...
      <div class="<?= !empty($errors['estimasi']) ? 'form-group-error' : '' ?>">
        ...
        <input type="text" name="estimasi" id="estimasi" class="input-field" placeholder="Otomatis terisi saat memilih layanan" value="<?= htmlspecialchars($data['estimasi'] ?? '') ?>" required>
